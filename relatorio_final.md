...
Foram implementadas duas funcionalidades novas:

- **Validador de CPF/CNPJ** com cálculo dos dígitos verificadores oficiais.
- **Validador de senha forte** com política configurável, que retorna a lista de critérios pendentes.

As duas são boas candidatas ao SDD por motivos diferentes:

- O validador de documentos tem regra fechada e externa ao projeto. O algoritmo da Receita Federal não admite interpretação, mas tem muitos casos de borda (máscara, sequências repetidas, tamanho errado, tipo errado) que se perdem quando se começa pelo código.
- O validador de senha tem a regra definida pelo próprio projeto, e quase todo o valor está no comportamento de fronteira: o que conta como caractere especial, se acento é letra, se oito espaços formam uma senha de oito caracteres. Essas perguntas não aparecem escrevendo `if len(senha) < 8`; aparecem escrevendo os cenários de aceite antes.
...
**OpenSpec.** Tem validação automática: `openspec validate --strict` recusa change sem delta, requisito sem cenário e cenário com o número errado de `#`. A validação pegou um erro real durante a atividade: ao traduzir os cabeçalhos dos artefatos para o português, o comando falhou com `No delta sections found`, mostrando que parte dos títulos é estrutura consumida por parser e não texto livre.

**SpecKit.** Não tem validação. Os templates vêm com marcadores `[NEEDS CLARIFICATION]` e nada impede entregar a spec com eles intactos. Em compensação, trouxe a constituição do projeto, com uma tabela de verificação no plano que obriga a confrontar cada princípio antes de implementar. Foi ela que forçou a justificar por escrito quais artefatos do template não seriam criados.
